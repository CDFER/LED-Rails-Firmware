...
# Trips to remove:
# 249-820312-75600-2-3M73342-b87d158a
...
ROUTE_SETS = {
    ...
    "WEST-201": {
        ...
        .union({i for i in range(142, 207)}),
        "COLOR": (0, 255, 0),
    },
    "STH-201": {
        "FILTER": "STH-201",
    ...
    "EAST-201": {
        "FILTER": "EAST-201",
        "END_DWELL": 300,
        "EXCLUDED_BLOCKS": {316, 158, 172, 169, 161},
        "COLOR": (255, 128, 0),
    },
}
    ...
    for schedule_key, entry in data.items():
        ...
        if filter_str == "WEST-201":
            ...
            if any(block in (343, 344) for _, block in averages):
                print(f"Setting {schedule_key} to Manukau Branch block order")
                block_order = (
                    list(range(344, 341, -1))  # 344-342 (descending)
                    + list(range(171, 139, -1))  # 171-140 (descending)
                    + list(range(305, 300, -1))  # 305-301 (descending)
                    + list(range(320, 314, -1))  # 320-315 (descending)
                )

                # Remove Manukau Branch from excluded blocks (344, 343, 342)
                trip_excluded_blocks -= {344, 343, 342}
        ...
        while i < len(averages):
            ...
            if block in trip_excluded_blocks or exclude:
                ...
                if adjusted_avg > prev_avg:
                    output_file.write(
                        f"\t\t{{ {int(adjusted_avg)}, {block} }}, // Was: {int(avg)} ({int(adjusted_avg - avg):+d} seconds)\n"
                    )
                ...
                if block == block_order[0] or block != block_order[1]:
                    output_file.write(f"\t\t{{ 0, {block} }}, // Was: {int(avg)}\n")
                    averages[i] = (0, block)
                else:
                    output_file.write(f"\t\t{{ 0, {block_order[0]} }}, // Added\n")
                    output_file.write(f"\t\t{{ {int(avg)}, {block} }},\n")
                i += 1
            else:
                output_file.write(f"\t\t{{ {int(avg)}, {block} }},\n")
...
if __name__ == "__main__":
    ...
    generate_cpp_header(
        ...
        f"include/{VERSION}_Timetable.h",
    )
